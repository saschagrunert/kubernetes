...
func (m *imageManager) EnsureOCIObject(
...
) (imageRef, message string, ociMount *kubecontainer.OCIMount, err error) {
	...
	if err != nil {
		...
		return "", msg, nil, ErrInvalidImageName
	}
	...
	if err != nil {
		...
		m.logIt(ref, v1.EventTypeWarning, events.FailedToInspectImage, logPrefix, msg, klog.Warning)
		return "", msg, nil, ErrImageInspect
	}
	...
	if !shouldPullImage(pullPolicy, present) {
		if present {
			...
			if mount {
				repull, msg, ociMount, err := m.getOCIObjectMount(ctx, pullType, logPrefix, pullPolicy, ref, spec, pod, ociReference)
				if err != nil {
					return "", msg, nil, err
				}
				if repull {
		...
		return "", msg, nil, ErrImageNeverPull
	}
...
pull:
	...
	if m.backOff.IsInBackOffSinceUpdate(backOffKey, m.backOff.Clock.Now()) {
		...
		return "", msg, nil, ErrImagePullBackOff
	}
	...
	m.backOff.GC()
	return imagePullResult.imageRef, "", imagePullResult.mount, nil
}

func (m *imageManager) getOCIObjectMount(
	ctx context.Context,
	pullType, logPrefix string,
	pullPolicy v1.PullPolicy,
	ref *v1.ObjectReference,
	spec kubecontainer.ImageSpec,
	pod *v1.Pod,
	ociReference string,
) (repull bool, message string, mount *kubecontainer.OCIMount, err error) {
	if !utilfeature.DefaultFeatureGate.Enabled(features.OCIVolume) {
		return false, "", nil, nil
	}

	spec.MountLabel = volumeutil.SELinuxOptionsToLabel(pod.Spec.SecurityContext.SELinuxOptions, false)

	mount, err = m.imageService.GetOCIObjectMount(ctx, spec)
	if err != nil {
		msg := fmt.Sprintf("Failed to get %s mount ref %q: %v", pullType, ociReference, err)
		m.logIt(ref, v1.EventTypeWarning, events.FailedToInspectImage, logPrefix, msg, klog.Warning)
		return false, msg, nil, ErrImageInspect
	}
	...
	if mount == nil {
		if pullPolicy != v1.PullNever {
			...
			m.logIt(ref, v1.EventTypeWarning, events.ErrImageNeverPullPolicy, logPrefix, msg, klog.Warning)
			return false, msg, nil, ErrImageNeverPull
		}
	}

	return false, "", mount, nil
}
