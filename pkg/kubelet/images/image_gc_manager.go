...
func (im *realImageGCManager) handleOCIVolumes(ctx context.Context, imagesInUse sets.Set[string], container *container.Container, pod *container.Pod, images []container.Image) error {
	...
	for _, mount := range status.Mounts {
		for _, image := range images {
			...
			if (image.MountRef == mount.ImageRef) || // mounted as image ref
				image.MountRef == mount.HostPath { // mounted as host path
				klog.V(5).InfoS("Container uses OCI object as mount", "pod", klog.KRef(pod.Namespace, pod.Name), "containerName", container.Name, "mountRef", image.MountRef, "imageID", image.ID)
