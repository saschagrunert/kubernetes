...
func makeMounts(pod *v1.Pod, podDir string, container *v1.Container, hostName, hostDomain string, podIPs []string, podVolumes kubecontainer.VolumeMap, hu hostutil.HostUtils, subpather subpath.Interface, expandEnvs []kubecontainer.EnvVar, supportsRRO bool, ociVolumes map[string]kubecontainer.OCIMount) ([]kubecontainer.Mount, func(), error) {
	...
	for i, mount := range container.VolumeMounts {
		...
		if ociMount, ok := ociVolumes[mount.Name]; ok {
			mount := kubecontainer.Mount{
				Name:          mount.Name,
				ContainerPath: mount.MountPath,
			}

			switch ociMount.Source {
			case runtimeapi.OCIVolumeMountSource_HOSTPATH:
				mount.HostPath = ociMount.Ref

			case runtimeapi.OCIVolumeMountSource_IMAGEREF:
				mount.ImageRef = ociMount.Ref

			default:
				return nil, cleanupAction, fmt.Errorf("unsupported OCI volume source: %s", ociMount.Source)
			}
...
func (kl *Kubelet) GenerateRunContainerOptions(ctx context.Context, pod *v1.Pod, container *v1.Container, podIP string, podIPs []string, ociVolumes map[string]kubecontainer.OCIMount) (*kubecontainer.RunContainerOptions, func(), error) {
	...
	mounts, cleanupAction, err := makeMounts(pod, kl.getPodDir(pod.UID), container, hostname, hostDomainName, podIPs, volumes, kl.hostutil, kl.subpather, opts.Envs, supportsRRO, ociVolumes)
	if err != nil {
		return nil, cleanupAction, err
	}
