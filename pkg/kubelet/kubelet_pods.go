...
func makeMounts(pod *v1.Pod, podDir string, container *v1.Container, hostName, hostDomain string, podIPs []string, podVolumes kubecontainer.VolumeMap, hu hostutil.HostUtils, subpather subpath.Interface, expandEnvs []kubecontainer.EnvVar, supportsRRO bool, ociVolumes map[string]string) ([]kubecontainer.Mount, func(), error) {
	...
	for i, mount := range container.VolumeMounts {
		if utilfeature.DefaultFeatureGate.Enabled(features.OCIVolume) {
			...
			if hostPath, ok := ociVolumes[mount.Name]; ok {
...
func (kl *Kubelet) GenerateRunContainerOptions(ctx context.Context, pod *v1.Pod, container *v1.Container, podIP string, podIPs []string, ociVolumes map[string]string) (*kubecontainer.RunContainerOptions, func(), error) {
